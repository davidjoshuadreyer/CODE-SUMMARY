...
assert.ok(hints('ret').includes('return'));
assert.equal(suggest('cou',3).items[0][1],'std::cout');
assert.equal(suggest('std::cou',8).items[0][1],'cout');
assert.equal(suggest('using namespace std; cou',24).items[0][1],'cout');
assert.ok(hints('int radius = 2; rad').includes('radius'));
assert.ok(hints('double area(double radius); ar').includes('area'));
for(const code of ['// ret','/* ret','"ret','R"tag(raw " ret','#include <str','unknown::ret','unknown.ret'])assert.equal(hints(code).length,0,code);
...
(async()=>{
 ...
 try{
  ...
  await editor.fill('');await editor.pressSequentially('cou');
  assert.ok(await page.locator('#member-suggestions').isVisible());await editor.press('Tab');assert.equal(await editor.inputValue(),'std::cout');
  await editor.fill('std::');assert.ok(await page.locator('#member-suggestions').isVisible());await editor.pressSequentially('cou');await editor.press('Enter');assert.equal(await editor.inputValue(),'std::cout');
  await editor.fill('int radius = 2;\nrad');await editor.press('Tab');assert.equal(await editor.inputValue(),'int radius = 2;\nradius');
  await editor.fill('ret');await editor.press('Enter');assert.equal(await editor.inputValue(),'return');
  await editor.fill('    fo');await page.locator('#member-suggestions').getByRole('option',{name:/Counted loop snippet/}).click();
  assert.equal(await editor.evaluate(e=>e.value.slice(e.selectionStart,e.selectionEnd)),'count');
  assert.match(await editor.inputValue(),/\n        \n    }$/);
  await editor.fill('std::sqrt(9)');await editor.evaluate(e=>e.setSelectionRange(7,7));await editor.press('Control+Space');await editor.press('Tab');assert.equal(await editor.inputValue(),'std::sqrt(9)');
  await editor.fill('cou');await editor.press('Escape');await editor.press('Enter');assert.equal(await editor.inputValue(),'cou\n');
  await editor.fill('cout << "ret');assert.ok(await page.locator('#member-suggestions').isHidden());
  await editor.fill('ret');await editor.press('Tab');await page.reload();await page.waitForFunction(()=>!document.querySelector('#run').disabled);assert.equal(await editor.inputValue(),'return');
