...
const resources = [];
const studyCourses={engr290:{data:require('../engr290/content.json'),sourceDir:'ENGR 290'},math250b:{data:require('./math250b-content.cjs'),sourceDir:'Math 250B'}};
for (const [dir,courseId] of [['chem','chem'],['matrix','matrix'],['c','c'],['Statistics','stats'],['engr290','engr290'],['math250b','math250b']]) {
 for (const file of fs.readdirSync(path.join(root,dir)).filter(f=>f.endsWith('.html')).sort()) {
  ...
  resources.push({id:'page-'+id(rel),courseId,title:lesson?lesson.title:titles[stem]||stem.replaceAll('_',' '),kind:stem==='quiz'?'Quiz':stem==='exam'?'Practice exam':stem.includes('Report')?'Lab report':'Study notes',url:url(rel),body:'',description:lesson?lesson.summary:'',referenceIds:study?(lesson?lesson.sources:study.data.sources).map(s=>'ref-'+id(study.sourceDir+'/'+s)):[],builtin:true});
 }
}
const references=[];
