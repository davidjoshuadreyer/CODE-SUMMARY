...
const courses = [
 ...
 {id:'comp139e',code:'COMP 139E',name:'C++ & Engineering Computing',semester:'Fall 2026',description:'Five covered lecture decks with ten simple programs, 18 tutorials, nine lab guides, and an online C++ editor with member suggestions.',symbol:'C++'}
];
...
for (const [dir,courseId] of [['math252','math252'],['chem','chem'],['matrix','matrix'],['c','c'],['Statistics','stats'],['engr290','engr290'],['math250b','math250b'],['comp139e','comp139e']]) {
 for (const file of fs.readdirSync(path.join(root,dir)).filter(f=>f.endsWith('.html')).sort()) {
  ...
  const study=studyCourses[courseId],lesson=courseId==='math252'?math252Lessons.find(l=>l.slug===stem):study?.data.lessons.find(l=>l.slug===stem);
  resources.push({id:'page-'+id(rel),courseId,title:courseId==='math252'?(lesson?lesson.title:stem==='lessons'?'Start here · Simplified lessons':'Original course materials'):lesson?lesson.title:courseTitles[stem]||titles[stem]||stem.replaceAll('_',' '),kind:stem==='quiz'?'Quiz':stem==='exam'?'Practice exam':stem.includes('Report')?'Lab report':'Study notes',url:url(rel),body:'',description:lesson?lesson.summary:'',referenceIds:courseId==='math252'?(lesson?lesson.sources.map(id=>'ref-math252-'+id):[]):study?(lesson?lesson.sources:study.data.sources).map(s=>'ref-'+id(study.sourceDir?study.sourceDir+'/'+s:s)):[],builtin:true});
 }
}
const references=[];
