...
const resources = [];
for (const [dir,courseId] of [['chem','chem'],['matrix','matrix'],['c','c'],['Statistics','stats']]) {
 for (const file of fs.readdirSync(path.join(root,dir)).filter(f=>f.endsWith('.html')).sort()) {
  ...
  resources.push({id:'page-'+id(rel),courseId,title:titles[stem]||stem.replaceAll('_',' '),kind:stem==='quiz'?'Quiz':stem==='exam'?'Practice exam':stem.includes('Report')?'Lab report':'Study notes',url:url(rel),body:'',description:'',referenceIds:[],builtin:true});
 }
}
const references=[];
function walk(dir,courseId){
 ...
 for(const entry of fs.readdirSync(path.join(root,dir),{withFileTypes:true})){
  ...
  else if(/\.pdf$/i.test(entry.name)) references.push({id:'ref-'+id(rel),courseId,name:entry.name,url:url(rel),size:fs.statSync(path.join(root,rel)).size,stage:'unreviewed',notes:'Existing reference. Check whether this was used in a study page, then link it here.',builtin:true,createdAt:'2026-01-01T00:00:00.000Z'});
 }
}
