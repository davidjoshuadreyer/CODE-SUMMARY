...
let md = fs.readFileSync(BASE + '/Lab10_AAS_Report.md', 'utf8');
const katexCSS = fs.readFileSync('./node_modules/katex/dist/katex.min.css', 'utf8');
const svgContent = fs.readFileSync(BASE + '/aas_calibration_curve.svg', 'utf8');
...
// Inject SVG directly into the HTML after parsing (avoids markdown mangling)
body = body.replace(/<img[^>]*aas_calibration_curve\.svg[^>]*>/i,
  `<div style="text-align:center;margin:18px 0">${svgContent}</div>`);

const css = `
@page{margin:0.75in 0.9in;size:letter;@top-center{content:none}@bottom-center{content:none}@bottom-left{content:none}@bottom-right{content:none}}
...
<head>
<meta charset="UTF-8">
<title>Lab 10 - Atomic Absorption Spectroscopy</title>
<style>
${katexCSS}
${css}
</style>
...
fs.writeFileSync(BASE + '/Lab10_AAS_Report.html', html, 'utf8');
